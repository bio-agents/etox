...
class UtilityExtension extends \Twig_Extension
{
    ...
    public function highlightEntitiesDocumentsKeywords($text,$sentenceId,$entityBackup, $field, $whatToSearch, $source, $entityType, $agenttipCounter)
    {
        ...
        $arrayReturn=$this->highlightEntitiesDocuments($text,$document,$entityBackup, $field, $whatToSearch, $source, $entityType, $agenttipCounter);
        return $arrayReturn;
    }

    public function highlightEntitiesDocuments($text,$document,$entityBackup, $field, $whatToSearch, $source, $entityType, $agenttipCounter)
    {
        //This function should return an array with both, $array[0] = the text highlighted, $array[1] = the html for the divs containing the sticky agenttips and $array[2] = the agenttipCounter number
        ...
        */
            ...
            if (strcasecmp($entityName, $entityBackup) != 0) {
                ...
                if($numberWords==1){
                    ...
                    foreach($arrayPlaces as $place){
                        ...
                        $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\"  class=\"atip\">$mouseoverSummary</div>";
                        ...
                        $text = str_ireplace($entityName, "<span data-agenttip=\"sticky$agenttipCounter\"><a href=\"$link\">".$entityName."</a></span>", $text);
                        $agenttipCounter=$agenttipCounter+1;
                    ...
                    if($place!=-1){
                        ...
                        if($place+$numberEntityName<=count($arrayText)){//Only in this case is possible to find the entityName inside the text, otherwise it will fall outside the array with undefined offset error
                            ...
                            //We generate the route to a free search with the entityName.
                            $link=$this->generator->generate('elasticSearch_keyword', array('whatToSearch' => 'any', 'source' => $source, 'keyword' => $entityName,));
                            $text = str_ireplace($arrayEntityName[0], "<span data-agenttip=\"sticky$agenttipCounter\"><a href=\"$link\">".$arrayEntityName[0], $text);
                            $agenttipCounter=$agenttipCounter+1;
                            $arrayText[$place]=$text;
                            //Mark the last
                ...
                if($numberWords==1){
                    ...
                    foreach($arrayPlaces as $place){
                        ...
                        $text = str_ireplace($entityBackup, "<span data-agenttip=\"sticky$agenttipCounter\"><a href=\"$link\">".$entityBackup."</a></span>", $text);
                        $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
        ...
        foreach ($arrayHepKeywordTermNorm2Document as $term2Document){
            ...
            if (strcasecmp($entityName, $entityBackup) != 0) {
                ...
                //$text = str_ireplace($entityName, "<span data-agenttip class='has-tip' data-options='touch_close_text:tap to close' title='".$mouseoverSummary."'>".$entityName."</span>", $text);
            }else{
                //We haven't changed color for entityBackup case insensitive search of entities. We change it now.
                //$text = str_ireplace($entityBackup, '<mark class="termSearched">'.$entityBackup.'</mark>', $text);
                $numberWords=str_word_count($entityName, 0, '0..9()=-');
            ...
            if (strcasecmp($entityName, $entityBackup) != 0) {
                    ...
                    case 'Marker':
                        ...
                        if($numberWords==1){
                            ...
                            foreach($arrayPlaces as $place){
                                ...
                                $text = str_ireplace($entityName, "<span data-agenttip=\"sticky$agenttipCounter\"><a href=\"$url\">".$entityName."</a></span>", $text);
                                $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
                            ...
                            if($place!=-1){
                                ...
                                if($place+$numberEntityName<=count($arrayText)){//Only in this case is possible to find the entityName inside the text, otherwise it will fall outside the array with undefined offset error_get_last()
                                    ...
                                    $mouseoverSummary=$em->getRepository('EtoxMicromeEntity2DocumentBundle:Entity2Document')->getEntitySummary($entity2DocumentId,"Marker");
                                    $text = str_ireplace($arrayEntityName[0], "<span data-agenttip=\"sticky$agenttipCounter\"><a href=\"$url\">".$arrayEntityName[0], $text);
                                    $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
                    ...
                    case 'Specie':
                        ...
                        if($numberWords==1){
                            ...
                            foreach($arrayPlaces as $place){
                                ...
                                $text = str_ireplace($entityName, "<span data-agenttip=\"sticky$agenttipCounter\">".$entityName."</span>", $text);
                                $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
                            ...
                            if($place!=-1){
                                ...
                                if($place+$numberEntityName<=count($arrayText)){//Only in this case is possible to find the entityName inside the text, otherwise it will fall outside the array with undefined offset error_get_last()
                                    ...
                                    $mouseoverSummary=$em->getRepository('EtoxMicromeEntity2DocumentBundle:Entity2Document')->getEntitySummary($entity2DocumentId,"Specie");
                                    $text = str_ireplace($arrayEntityName[0], "<span data-agenttip=\"sticky$agenttipCounter\">".$arrayEntityName[0], $text);
                                    $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
                ...
                if($numberWords==1){
                    ...
                    foreach($arrayPlaces as $place){
                        ...
                        $text = str_ireplace($entityBackup, "<span data-agenttip=\"sticky$agenttipCounter\"><a href=\"$url\">".$entityBackup."</a></span>", $text);
                        $arrayText[$place]=$text;
                        $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
        ...
        foreach($arraySpecie2Document as $specie2document){
            ...
            if($numberWords==1){
                ...
                foreach($arrayPlaces as $place){
                    ...
                    $text = str_ireplace($specieName, "<span data-agenttip=\"sticky$agenttipCounter\">".$specieName."</span>", $text);
                    $arrayText[$place]=$text;
                ...
                if($place!=-1){
                    ...
                    if($place+$numberSpecieName<=count($arrayText)){//Only in this case is possible to find the entityName inside the text, otherwise it will fall outside the array with undefined offset error_get_last()
                        ...
                        $text = str_ireplace($arraySpecieName[0], "<span data-agenttip=\"sticky$agenttipCounter\">".$arrayEntityName[0], $text);
                        $arrayText[$place]=$text;
                        $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
                        $agenttipCounter=$agenttipCounter+1;
                        //Mark the last
        ...
        $arrayReturn[2]=$agenttipCounter;
        return ($arrayReturn);
    }

    public function highlightEntitiesAbstracts($text,$abstract,$entityBackup,$field, $whatToSearch, $source, $entityType, $agenttipCounter)
    {
            ...
            if (strcasecmp($entityName, $entityBackup) != 0) {
                ...
                if($numberWords==1){
                    ...
                    foreach($arrayPlaces as $place){
                        ...
                        $text = str_ireplace($entityBackup, "<span data-agenttip=\"sticky$agenttipCounter\">".$entityBackup."</span>", $text);
                        $arrayText[$place]=$text;
                        $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
                        $agenttipCounter=$agenttipCounter+1;
                    }
    ...
    public function highlightGenesAbstracts($text,$abstract,$entityBackup,$field, $whatToSearch, $source, $entityType, $agenttipCounter, $orderBy)
    {
            ...
            if (strcasecmp($geneName, $entityBackup) != 0) {//We just highlight the entities that are not the same as the gene that is beeing searched
                ...
                if($numberWords==1){
                    ...
                    foreach($arrayPlaces as $place){
                        ...
                        $text = str_ireplace($geneName, "<span data-agenttip=\"sticky$agenttipCounter\"><a href=\"$url\">".$geneName."</a></span>", $text);
                        $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
                        $agenttipCounter=$agenttipCounter+1;
                        $arrayText[$place]=$text;
                    }
                    ...
                    if($place!=-1){
                        ...
                        if($place+$numberGeneName<=count($arrayText)){//Only in this case is possible to find the geneName inside the text, otherwise it will fall outside the array with undefined offset error_get_last()
                            ...
                            $text = str_ireplace($arrayGeneName[0], "<span data-agenttip=\"sticky$agenttipCounter\">".$arrayGeneName[0], $text);
                            $mouseoverDivs=$mouseoverDivs."<div id=\"sticky$agenttipCounter\" class=\"atip\">$mouseoverSummary</div>";
                            $agenttipCounter=$agenttipCounter+1;
                            $arrayText[$place]=$text;
                            //Mark the last
